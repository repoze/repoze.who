...
from wsgiref import util as wsgiref_util # setup_testing_defaults
...
def test_atcp_ctor_timeout_lower_than_reissue():
    with pytest.raises(ValueError):
        auth_tkt.AuthTktCookiePlugin(
            SECRET, 'userid', timeout=1, reissue_time=2,
        )
...
def test_atcp_identify_good_cookie_include_ip():
    ...
    val = _makeTicket(remote_addr='1.1.1.1', userdata='foo=123')
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % val}
...
def test_atcp_identify_good_cookie_dont_include_ip():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, include_ip=False)
...
def test_atcp_identify_good_cookie_int_useridtype():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, include_ip=False)
    val = _makeTicket(userid='1', userdata='userid_type=int')
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % val}
...
def test_atcp_identify_good_cookie_unknown_useridtype():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, include_ip=False)
    val = _makeTicket(userid='userid', userdata='userid_type=unknown')
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % val}
...
def test_atcp_identify_bad_cookie_expired():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, timeout=2, reissue_time=1)
    val = _makeTicket(userid='userid', time=time.time()-3)
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % val}
...
def test_atcp_identify_with_checker_and_existing_account():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, userid_checker=dummy_userid_checker)
    val = _makeTicket(userid='existing', userdata='foo=123')
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % val}
...
def test_atcp_identify_with_alternate_hash():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, include_ip=False, digest_algo="sha256")
    val = _makeTicket(userdata='foo=123', digest_algo="sha256")
...
def test_atcp_identify_bad_cookie_with_alternate_hash():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, include_ip=True, digest_algo="sha256")
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=bogus'}
...
def test_atcp_remember_creds_hash_mismatch():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, digest_algo="sha1")
    old_val = _makeTicket(userid='userid', userdata='foo=123', digest_algo="md5")
    new_val = _makeTicket(userid='userid', userdata='foo=123', digest_algo="sha1")
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % old_val}
...
def test_atcp_remember_creds_secure_alternate_hash():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, secure=True, digest_algo="sha512")
    val = _makeTicket(userid='userid', secure=True, userdata='foo=123', digest_algo="sha512")
    environ = _make_wsgi_environ()
...
def test_atcp_remember_creds_secure():
    ...
    val = _makeTicket(userid='userid', secure=True, userdata='foo=123')
    environ = _make_wsgi_environ()
...
def test_atcp_remember_creds_samesite():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, secure=False, samesite="Strict")
    val = _makeTicket(userid='userid', secure=False, userdata='foo=123')
    environ = _make_wsgi_environ()
...
def test_atcp_remember_creds_different_include_ip():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, include_ip=True)
    old_val = _makeTicket(userid='userid', remote_addr='1.1.1.1')
    environ = _make_wsgi_environ() | {'HTTP_COOKIE': 'auth_tkt=%s' % old_val}
    new_val = _makeTicket(userid='other',
                                userdata='foo=123',
                                remote_addr='1.1.1.1')
...
def test_atcp_remember_creds_different_with_tokens():
    ...
    old_val = _makeTicket(userid='userid')
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % old_val}
    new_val = _makeTicket(userid='userid',
                                userdata='foo=123',
                                tokens=['foo', 'bar'],
                                )
...
def test_atcp_remember_creds_different_int_userid():
    ...
    old_val = _makeTicket(userid='userid')
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % old_val}
    new_val = _makeTicket(userid='1', userdata='userid_type=int')
...
def test_atcp_remember_creds_different_unicode_userid():
    ...
    userdata = ''
    new_val = _makeTicket(userid=userid.encode('utf-8'),
                                userdata=userdata)
...
def test_atcp_remember_creds_reissue():
    ...
    environ = _make_wsgi_environ() | {'HTTP_COOKIE':'auth_tkt=%s' % old_val}
    new_val = _makeTicket(userid='userid', userdata='')
...
def test_atcp_remember_creds_reissue_alternate_hash():
    plugin = auth_tkt.AuthTktCookiePlugin(SECRET, reissue_time=1, digest_algo="sha256")
    old_val = _makeTicket(userid='userid', userdata='',
...
def test_atcp_remember_l10n_sane_cookie_date():
    ...
    assert value.startswith('auth_tkt="%s"; Path=/; Max-Age=500' % tkt)
