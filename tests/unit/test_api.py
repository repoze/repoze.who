...
def test_match_classification():
    ...
    # specific
    assert (
        api_module.match_classification(
            interfaces.IIdentifier, plugins, 'foo',
        ) == [multi1, multi2]
    )
    assert(
        api_module.match_classification(
            interfaces.IIdentifier, plugins, 'bar',
        ) == [multi1]
    )
    assert(
        api_module.match_classification(
            interfaces.IIdentifier, plugins, 'biz',
        ) == [multi2]
    )
    # any for multi2
    assert(
        api_module.match_classification(
            interfaces.IChallenger, plugins, 'buz',
        ) == [multi1, multi2]
    )
    # any for either
    assert(
        api_module.match_classification(
            interfaces.IAuthenticator, plugins, 'buz',
        ) == [multi1, multi2]
    )
...
def test_api_challenge_w_multi_challengers_firstwins():
    ...
    api = _make_API(environ=environ, challengers=plugins,
                        request_classifier=lambda environ: 'match')
...
def test_api_login_w_identifier_name_hit():
    ...
    api = _make_API(identifiers=identifiers,
...
def test_api__authenticate_success_skip_fail():
    ...
    api = _make_API(
        environ=environ, authenticators=plugins, logger=logger,
    )
    ...
    assert logger._debug[0].startswith(
    ...
    assert logger._debug[4].startswith(
