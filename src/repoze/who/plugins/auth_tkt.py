...
class ExactlyOneOfSecretOrSecretFile(ValueError):
    def __init__(self):
        super().__init__("Pass exactly one of 'secret' or 'secretfile'")
...
class AuthTktCookiePlugin:
    ...
    def remember(self, environ, identity):
        ...
        if old_data != new_data or (
            self.reissue_time
            and ((timestamp + self.reissue_time) < time.time())
        ):
    ...
    def _get_cookies(self, environ, value, max_age=None):
        ...
        cookies = [
            (
                "Set-Cookie",
                f'{self.cookie_name}="{value}"; Path=/{max_age}{secure}',
            ),
            (
                "Set-Cookie",
